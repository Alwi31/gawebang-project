...
class ProjectController extends Controller
{
    ...
    public function index()
    {
        ...
        if ($user->hasRole('project_client')) {
            // filtering data project berdasarkan dari client id == user_id
    ...
    public function store(StoreProjectRequest $request)
    {
        ...
        DB::transaction(function () use ($request, $user) {
            ...
            // create wallet transaction for project cost 
            $projectWalletTransaction = WalletTransaction::created([
                'type' => 'Project Cost',
    ...
    public function tools_store(StoreToolProjectRequest $request, Project $project)
    {
        DB::transaction(function () use ($request, $project) {
            // Your tool addition logic here
            $validated = $request->validated();
            $validated['project_id'] = $project->id;

            $toolProject = ProjectTool::create($validated);

            $newProjectTool = ProjectTool::firstOrCreate($validated);
        });
