        ...
        <div class="max-w-5xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-10 flex flex-col gap-y-5">
                ...
                @if($walletTransaction->type == 'Withdraw')
                    ...
                    <form method="POST" action="{{route('admin.wallet_transaction.update', $walletTransaction)}}" enctype="multipart/form-data">
                        @csrf
                        @method('PUT')
                        <div class="mt-4">
                ...
                <img src="{{Storage::url($walletTransaction->proof)}}" alt=""
                    ...
                    @if(!$walletTransaction->is_paid)
                        <hr class="my-5">
                        <form action="{{route('admin.wallet_transaction.update', $walletTransaction)}}" method="POST">
                            @csrf
                            @method('PUT')
                            <button type="submit" class="font-bold py-4 px-6 bg-indigo-700 text-white rounded-full">
                                Approve Topup
