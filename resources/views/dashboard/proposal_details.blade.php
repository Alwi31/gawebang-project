<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8">
            <div
                class="bg-white overflow-hidden shadow-sm sm:rounded-lg p-6 sm:p-8 lg:p-10 flex flex-col gap-6 sm:gap-8">

                <!-- Status Alert Banner -->
                @if ($project->has_finished)
                    @if ($projectApplicant->status == 'Hired')
                        <div
                            class="text-white font-bold bg-green-500 rounded-2xl w-full p-4 sm:p-5 text-sm sm:text-base">
                    ...
                    @endif
                @else
                    <div class="text-white font-bold bg-cyan-500 rounded-2xl w-full p-4 sm:p-5 text-sm sm:text-base">
                ...
                <!-- Section Header -->
                <div>
                    <h3 class="text-indigo-950 text-lg sm:text-xl font-bold">You've applied to this job</h3>
                </div>

                <!-- Project Card -->
                <div
                    class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 sm:gap-6 p-4 sm:p-5 border border-slate-200 rounded-lg">
                    <div class="flex flex-row items-center gap-3 sm:gap-4 flex-1">
                        <img src="{{ Storage::url($project->thumbnail) }}" alt=""
                            class="rounded-lg sm:rounded-2xl object-cover w-20 h-16 sm:w-28 sm:h-20 flex-shrink-0">
                        <div class="flex flex-col min-w-0">
                            <h3 class="text-indigo-950 text-base sm:text-lg font-bold truncate">{{ $project->name }}
                            </h3>
                            <p class="text-slate-500 text-xs sm:text-sm">{{ $project->category->name }}</p>
                        </div>
                    </div>
                    <a href="{{ route('front.details', $project->slug) }}"
                        class="font-bold py-2 sm:py-3 px-4 sm:px-6 text-center bg-indigo-700 text-white rounded-full text-sm sm:text-base hover:bg-indigo-800 transition-colors w-full sm:w-auto">
                        View Details
                    </a>
                </div>
                ...
                <div
                    ...
                    <div class="flex flex-row items-center gap-3 sm:gap-4 flex-1">
                        <img src="{{ Storage::url($projectApplicant->freelancer->avatar) }}" alt=""
                            class="rounded-full object-cover w-16 h-16 sm:w-20 sm:h-20 flex-shrink-0">
                        <div class="flex flex-col min-w-0">
                            <h3 class="text-indigo-950 text-base sm:text-lg font-bold truncate">
                                {{ $projectApplicant->freelancer->name }}</h3>
                            <p class="text-slate-500 text-xs sm:text-sm">{{ $projectApplicant->freelancer->occupation }}
                            </p>
                        </div>
                    </div>

                    <!-- Status Badge -->
                    <div class="flex-1 sm:flex-none sm:min-w-max">
                        @if ($projectApplicant->status == 'Hired')
                            <span
                                class="inline-block text-xs sm:text-sm font-bold py-2 px-3 sm:px-4 rounded-full bg-green-500 text-white">
                                YOU'RE HIRED
                            </span>
                        @elseif($projectApplicant->status == 'Waiting')
                            <span
                                class="inline-block text-xs sm:text-sm font-bold py-2 px-3 sm:px-4 rounded-full bg-orange-500 text-white">
                                WAITING FOR APPROVAL
                            </span>
                        @elseif($projectApplicant->status == 'Rejected')
                            <span
                                class="inline-block text-xs sm:text-sm font-bold py-2 px-3 sm:px-4 rounded-full bg-red-500 text-white">
                                REJECTED
                            </span>
                        @endif
                    </div>
                ...
                <!-- Message & Date -->
                <div>
                    <p class="text-indigo-950 text-base sm:text-lg font-medium leading-relaxed">
                ...
                @if ($projectApplicant->status == 'Hired')
                    ...
                    <!-- You're Hired! Info -->
                    <div
                        class="flex flex-col sm:flex-row gap-4 sm:gap-4 items-start sm:items-center p-4 sm:p-5 bg-green-50 border border-green-200 rounded-lg">
                        ...
                        <div class="flex-1">
                            <h3 class="text-green-900 text-lg sm:text-xl font-bold">You're Hired!</h3>
                    ...
                    <div>
                        <h3 class="text-indigo-950 text-lg sm:text-xl font-bold mb-4">Tired of Waiting? Contact Client
                            Now</h3>
                        <div
                            class="flex flex-row gap-4 items-center border border-slate-200 p-4 sm:p-5 rounded-lg w-full sm:w-fit hover:bg-slate-50 transition-colors">
                            ...
                            <p class="text-indigo-950 text-base sm:text-lg font-bold break-all">
                    ...
                    <!-- Income Info -->
                    <div
                        class="flex flex-col sm:flex-row gap-4 sm:gap-4 items-start sm:items-center p-4 sm:p-5 bg-blue-50 border border-blue-200 rounded-lg">
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg" class="flex-shrink-0">
                            <path opacity="0.8"
                        ...
                        <div class="flex-1">
                            <h3 class="text-blue-900 text-lg sm:text-xl font-bold">Income Rp
                ...
                @elseif($projectApplicant->status == 'Waiting')
                    <!-- Waiting State - Empty or minimal message -->
                @elseif($projectApplicant->status == 'Rejected')
                    <hr class="border-slate-200">

                    <!-- Rejected Info -->
                    <div
                        class="flex flex-col sm:flex-row gap-4 sm:gap-4 items-start sm:items-center p-4 sm:p-5 bg-red-50 border border-red-200 rounded-lg">
                        <svg width="48" height="48" viewBox="0 0 24 24" fill="none"
                            xmlns="http://www.w3.org/2000/svg" class="flex-shrink-0">
                            <path
                        ...
                        <div class="flex-1">
                            <h3 class="text-red-900 text-lg sm:text-xl font-bold">Sorry, Client Rejected You.</h3>
                            <p class="text-red-700 text-sm sm:text-base mt-1">
